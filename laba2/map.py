...
class Map:
    ...
    def fill_grid_with_game_objects(self):
        ...
        red_amount = empty_positions_amount // 10
        orange_amount = empty_positions_amount // 8
        green_amount = empty_positions_amount // 8
        coins_amount = empty_positions_amount // 10
        empty_positions = self.get_empty_blocks_positions()
        for _ in range(red_amount):
            position = random.choice(empty_positions)
            self.grid[position] = RED_ZONE
        for _ in range(green_amount):
            position = random.choice(empty_positions)
            self.grid[position] = ORANGE_ZONE
        for _ in range(orange_amount):
            position = random.choice(empty_positions)
            self.grid[position] = GREEN_ZONE
