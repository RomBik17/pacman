...
class Pacman:
    ...
    def min_max(self):
        ...
        def maximize(game_state, depth, alpha, beta):
            ...
            allowed_actions = game_state.get_legal_actions(PACMAN)
            best_score = -999999
            temp_score = best_score
            best_action = vec(0, 0)
            for action in allowed_actions:
                new_state = game_state.simulate_state(action, PACMAN)
    ...
    def expect_max(self):
        ...
        def maximize(state, depth):
            if state.is_lose() or state.is_win():
                return state.get_score()
            actions = state.get_legal_actions(PACMAN)
            best_score = -99999
            score = best_score
            best_action = vec(0, 0)
            for action in actions:
                new_state = state.simulate_state(action, PACMAN)
